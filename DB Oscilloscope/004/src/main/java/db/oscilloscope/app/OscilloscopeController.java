...
public class OscilloscopeController {
    ...
    private void updateRbwChoices() {
        ...
        // Offer RBW values from 1 Hz to 100 Hz, ensuring fftSize is a power of 2
    ...
    private void drawWaveforms() {
        ...
        if (isSpectrumMode) {
            ...
            double dynamicRange = dynamicRangeSlider.getValue(); // dB (negative)
            ...
            fftSize = Math.min(Math.max(fftSize, 256), 8192); // Limit fftSize to 256–8192

            // Prepare data for FFT (zero-pad or truncate if needed)
            ...
            // Compute FFT with dynamic fftSize
            ...
            // Draw frequency domain grid (magnitude and phase)
            gc.setStroke(Color.rgb(100, 80, 50, 0.5));
            gc.setLineWidth(0.8);
            double magHeight = canvas.getHeight() / 2; // Top half for magnitude
            double phaseHeight = canvas.getHeight() / 2; // Bottom half for phase
            // Magnitude grid
            ...
            double minFreq = isLogScale ? Math.max(1, maxFreq / 1000) : 0; // Avoid log(0)
            for (int i = 0; i <= 10; i++) {
            ...
            double maxMagnitude = isDbScale ? 0 : 1; // For linear mode, normalize to 1
            ...
            if (isDbScale) {
                ...
                maxMagnitude = Math.min(maxMagnitude, 0); // Cap at 0 dB
            } else if (isCompareSpectra) {
                // In linear combined mode, find max magnitude across both channels
                for (int i = 0; i < ch1Spectrum.magnitude.length; i++) {
            ...
            for (int i = 0; i < ch1Spectrum.magnitude.length; i++) {
                ...
                if (isDbScale && mag < dynamicRange) mag = dynamicRange; // Clip to dynamic range floor
                double y;
                if (isCompareSpectra) {
                    // Use common scale based on ch1Volt and maxMagnitude
                    y = isDbScale ?
            ...
            // Draw peak markers and annotations
            ...
            if (showDigital) {
                ...
                // Draw CH1 digital trace
                gc.setStroke(Color.rgb(255, 255, 100, 0.7));
                double ch1DigitalY = ch1Pos + height / 4; // Offset below CH1 analog
                gc.beginPath();
                for (int i = 0; i < samples; i++) {
                    double x = i * canvas.getWidth() / samples;
                ...
                // Draw CH2 digital trace
                gc.setStroke(Color.rgb(100, 255, 255, 0.7));
                double ch2DigitalY = ch2Pos - height / 4; // Offset above CH2 analog
                gc.beginPath();
                for (int i = 0; i < samples; i++) {
                    double x = i * canvas.getWidth() / samples;
                    ...
                    if (i == 0) {
                        ...
                        if (ch2Digital[i] != ch2Digital[i - 1]) {
    ...
    private int findTriggerPoint(double[] data, double level, boolean rising) {
        ...
        // Fallback: Return 0 to ensure continuous updates
        return 0;
    }
